...
public class GooglePayIssuer extends CordovaPlugin {
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) {
    ...
    if ("getTokenStatus".equals(action)) {
      ...
      return true;
    } else if ("isTokenized".equals(action)) {
      this.cordova.getThreadPool().execute(new Runnable() {
        @Override
        public void run() {
...
//                                            joReturn.put("")
  ...
  private void isTokenized(String tsp, String lastDigits, CallbackContext callbackContext) {
      ...
      tapAndPayClient
        ...
        .addOnCompleteListener(
          new OnCompleteListener<Boolean>() {
            ...
            public void onComplete(@NonNull Task<Boolean> task) {
              Log.i(TAG, "onComplete (isTokenized) - " + task.isSuccessful());
              if (task.isSuccessful()) {
                Boolean isTokenized = task.getResult();
                callbackContext.success(isTokenized.toString());
              } else {
                callbackContext.error("error");
              }
            }
        ...
        .addOnFailureListener(
          new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
              Log.i(TAG, "onFailure (isTokenized) - " + e.getMessage());
              callbackContext.error("error");
            }
          })
        .addOnCanceledListener(
          new OnCanceledListener() {
            @Override
            public void onCanceled() {
              Log.i(TAG, "onCanceled (isTokenized) - ");
              callbackContext.error("error");
            }
          });
